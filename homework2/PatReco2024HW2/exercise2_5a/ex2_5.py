...
Date: 24/6/2024
Description: This script creates a 2-Layer Neural Network using the  Activations Functions Sigmoid,Softmax,Tanh
and the Error Loss Functions Cross-Entropy, MSE
"""
...
def train(network, x_train, y_train, epochs = 1000, learning_rate = 0.1, batch_size = 128, verbose = True):
    num_batches = 0
...
# Define different learning rates, epochs and batch_size to experiment with 128  batch size
...
toterror = sum([loss_cross_entropy(y, predict(network, x)) for x, y in zip(x_test, y_test)]) / len(x_test)
print('ratio: %.2f' % ratio)
# print('mean loss: %.4f' % toterror)
